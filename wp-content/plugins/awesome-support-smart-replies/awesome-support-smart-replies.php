<?php
/**
 * @package   Awesome Support: Smart Replies (A Machine Learning Engine For Awesome Support)
 * @author    AwesomeSupport
 * @link      http://www.getawesomesupport.com
 *
 * Plugin Name:       Awesome Support: Smart Replies Powered by AI and Machine Learning
 * Plugin URI:        
 * Description:       An add-on for Awesome Support that provides a series of options for responding to chat messages and new tickets using algorithmic searches (optionally powered by machine learning and artificial intelligence algorithms.)
 * Version:           4.1.2
 * Author:            Awesome Support Team
 * Author URI:        
 * Text Domain:       wpas_chatbot
 * License:           GPL-2.0+
...
register_activation_hook( __FILE__, array( 'WPAS_Chat_Bot', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'WPAS_Chat_Bot', 'deactivate' ) ) ;
add_action( 'plugins_loaded', array( 'WPAS_Chat_Bot', 'get_instance' ), 12 );
/**
...
class WPAS_Chat_Bot {
	...
	public function declare_constants() {
		define( 'WPAS_CBOT_VERSION', '4.1.2' );
		define( 'WPAS_CBOT_URL', trailingslashit( plugin_dir_url( __FILE__ ) ) );
		define( 'WPAS_CBOT_PATH', trailingslashit( plugin_dir_path( __FILE__ ) ) );
		define( 'WPAS_CBOT_ROOT', trailingslashit( dirname( plugin_basename( __FILE__ ) ) ) );
		define( 'WPAS_CBOT_NS', 'wpascbot_' );
	}
	...
	public static function activate() {
		if ( ! class_exists( 'Awesome_Support' ) ) {
			...
			wp_die(
				sprintf( __( 'You need Awesome Support to activate this addon. Please <a href="%s" target="_blank">install Awesome Support</a> before continuing.', 'wpas_chatbot' ), esc_url( 'https://getawesomesupport.com/?utm_source=internal&utm_medium=addon_loader&utm_campaign=Addons' ) )
			);
		} else {
			require_once trailingslashit( plugin_dir_path( __FILE__ ) ) . 'includes/functions.php';
			wpas_cbot_add_links_search_page();
		}
	}
	...
	public function init() {
		...
		if ( ! $this->is_core_active() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support to be active. Please activate the core plugin first.', 'wpas_chatbot' ), $plugin_name ) );
		}

		if ( ! $this->is_php_version_enough() ) {
			$this->add_error( sprintf( __( 'Unfortunately, %s can not run on PHP versions older than %s. Read more information about <a href="%s" target="_blank">how you can update</a>.', 'wpas_chatbot' ), $plugin_name, $this->php_version_required, esc_url( 'http://www.wpupdatephp.com/update/' ) ) );
		}

		if ( ! $this->is_version_compatible() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support version %s or greater. Please update the core plugin first.', 'wpas_chatbot' ), $plugin_name, $this->version_required ) );
		}
	...
	public static function deactivate() {
		...
		require_once WPAS_CBOT_PATH . 'includes/functions.php';
		wpas_cbot_remove_links_search_page();
	}
	...
	public function add_license_notice() {
		...
		WPAS()->admin_notices->add_notice( 'error', "license_{$this->slug}", sprintf( __( 'Please <a href="%s">fill-in your product license</a> now. If you don\'t, your copy of Awesome Support: Facebook Chat Bot <strong>will never be updated</strong>.', 'wpas_chatbot' ), $link ) );
	...
	public function license_notice_meta( $plugin_meta, $plugin_file ) {
		...
		if ( plugin_basename( __FILE__ ) === $plugin_file ) {
			$plugin_meta[] = '<strong>' . sprintf( __( 'You must fill-in your product license in order to get future plugin updates. <a href="%s">Click here to do it</a>.', 'wpas_chatbot' ), $license_page ) . '</strong>';
		}
	...
	public function load() {
		...
		require_once WPAS_CBOT_PATH . 'includes/class-keyword.php';
		require_once WPAS_CBOT_PATH . 'includes/settings.php';
		require_once WPAS_CBOT_PATH . 'includes/class-chatbot.php';
		
		
		require_once WPAS_CBOT_PATH . 'includes/smart_chat/settings.php';
		require_once WPAS_CBOT_PATH . 'includes/smart_chat/smart-chat.php';
		
		require_once WPAS_CBOT_PATH . 'includes/smart_reply/settings.php';
		require_once WPAS_CBOT_PATH . 'includes/smart_reply/class-smart-reply.php';
		
		require_once WPAS_CBOT_PATH . 'includes/fb/class-fb-chatbot.php';
		require_once WPAS_CBOT_PATH . 'includes/fb/class-fb-chatbot-messaging.php';
		
		require_once WPAS_CBOT_PATH . 'includes/ai/class-api-ai.php';
		require_once WPAS_CBOT_PATH . 'includes/ai/class-api-ai-fulfillment.php';
		
		require_once WPAS_CBOT_PATH . 'includes/gnl/gnl-functions.php';
		require_once WPAS_CBOT_PATH . 'includes/gnl/class-google-natural-language.php';
		
		add_action( 'tf_init', array( $this, 'tf_init' ) );
		
		new WPAS_CBOT_API_AI_Fulfillment();
		new WPAS_FB_CBOT();
	...
	/**
	 * Include custom Titan Framework option
	 *
	 * @since 1.0
	 * @return void
	 */
	public function tf_init() {
		...
		require_once WPAS_CBOT_PATH . 'includes/titan_fields/class-option-add-cbot-keyword.php';
		require_once WPAS_CBOT_PATH . 'includes/titan_fields/class-option-multi-cbot-keyword.php';
		require_once WPAS_CBOT_PATH . 'includes/titan_fields/class-option-multi-text.php';
		require_once WPAS_CBOT_PATH . 'includes/titan_fields/class-option-custom-textarea.php';
		require_once WPAS_CBOT_PATH . 'includes/titan_fields/class-option-search-post-type.php';
	...
	public function admin_enqueue_scripts() {
		if ( is_admin() ) {
			...
			wp_enqueue_script( 'wpas-cbot-script', WPAS_CBOT_URL . 'assets/js/admin/script.js', array( 'jquery' ) );
			wp_enqueue_style('wpas-cbot-style', WPAS_CBOT_URL . 'assets/css/style.css');
		}
	}
	...
	public function load_plugin_textdomain() {
		...
		$lang_dir       = WPAS_CBOT_ROOT . 'languages/';
		$lang_path      = WPAS_CBOT_PATH . 'languages/';
		$locale         = apply_filters( 'plugin_locale', get_locale(), 'wpas_chatbot' );
		$mofile         = "wpas_chatbot-$locale.mo";
		$glotpress_file =  WPAS_CBOT_PATH . $mofile;

		// Look for the GlotPress language pack first of all
		if ( file_exists( $glotpress_file ) ) {
			$language = load_textdomain( 'wpas_chatbot', $glotpress_file );
		} elseif ( file_exists( $lang_path . $mofile ) ) {
			$language = load_textdomain( 'wpas_chatbot', $lang_path . $mofile );
		} else {
			$language = load_plugin_textdomain( 'wpas_chatbot', false, $lang_dir );
		}
