 ...
 *
 * @wordpress-plugin
 * Plugin Name:       Awesome Support: Documentation & Knowledgebase
 * Plugin URI:        http://getawesomesupport.com/addons/documentation/
 * Description:       Easily create documentation for your products. The add-on is compatible with single and multi products.
 * Version:           3.0.2
 * Author:            Awesome Support
 * Author URI:        http://getawesomesupport.com
 * Text Domain:       wpas-documentation
 * License:           GPL-2.0+
...
define( 'WPAS_DOC_VERSION', '3.0.2' );
define( 'WPAS_DOC_URL',     trailingslashit( plugin_dir_url( __FILE__ ) ) );
define( 'WPAS_DOC_PATH',    trailingslashit( plugin_dir_path( __FILE__ ) ) );
 ...
 */
register_activation_hook( __FILE__, array( 'AS_Documentation_Loader', 'activate' ) );

add_action( 'plugins_loaded', array( 'AS_Documentation_Loader', 'get_instance' ) );
/**
...
class AS_Documentation_Loader {
	 ...
	 * @var    string
	 */
	protected $version_required = '3.3'; // update to 4.0 after 4.0 is released
	 ...
	 * @var    string
	 */
	protected $slug = 'wpas-documentation';
	...
	public static function activate() {
		...
		if ( ! class_exists( 'Awesome_Support' ) ) {
			...
			wp_die(
				sprintf( __( 'You need Awesome Support to activate this addon. Please <a href="%s" target="_blank">install Awesome Support</a> before continuing.', 'wpas-documentation' ), esc_url( 'http://getawesomesupport.com/?utm_source=internal&utm_medium=addon_loader&utm_campaign=Addons' ) )
			);
		}
	...
	public function init() {
		...
		if ( ! $this->is_core_active() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support to be active. Please activate the core plugin first.', 'wpas-documentation' ), $plugin_name ) );
		}

		if ( ! $this->is_php_version_enough() ) {
			$this->add_error( sprintf( __( 'Unfortunately, %s can not run on PHP versions older than %s. Read more information about <a href="%s" target="_blank">how you can update</a>.', 'wpas-documentation' ), $plugin_name, $this->php_version_required, esc_url( 'http://www.wpupdatephp.com/update/' ) ) );
		}

		if ( ! $this->is_version_compatible() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support version %s or greater. Please update the core plugin first.', 'wpas-documentation' ), $plugin_name, $this->version_required ) );
		}
	...
	public function addon_license( $licenses ) {
		...
		$plugin_name = 'Documentation'; // Remove the Awesome Support prefix from the addon name
	...
	public function license_notice() {
		...
		<div class="updated error">
			<p><?php printf( __( 'Please <a href="%s">fill-in your product license</a> now. If you don\'t, your copy of Awesome Support: Documentation <strong>will never be updated</strong>.', 'wpas-documentation' ), $license_page ); ?>
	...
	public function license_notice_meta( $plugin_meta, $plugin_file ) {
		...
		if ( plugin_basename( __FILE__ ) === $plugin_file ) {
			$plugin_meta[] = '<strong>' . sprintf( __( 'You must fill-in your product license in order to get future plugin updates. <a href="%s">Click here to do it</a>.', 'wpas-documentation' ), $license_page ) . '</strong>';
		}
	...
	public function load() {
		 ...
		 * Load widget classes
		 */
		require_once( WPAS_DOC_PATH . 'includes/widgets/table-of-contents/table-of-contents.php' );

		require_once( WPAS_DOC_PATH . 'class-documentation.php' );
		require_once( WPAS_DOC_PATH . 'includes/class-admin.php' );
		require_once( WPAS_DOC_PATH . 'includes/functions-templating.php' );

		require_once( WPAS_DOC_PATH . 'includes/functions-titan-framework.php' );
		require_once( WPAS_DOC_PATH . 'includes/functions-settings.php' );
		require_once( WPAS_DOC_PATH . 'includes/functions-documentation.php' );
		require_once( WPAS_DOC_PATH . 'includes/functions-quick-reply.php' );
		require_once( WPAS_DOC_PATH . 'includes/functions-live-search.php' );
		require_once( WPAS_DOC_PATH . 'includes/functions-search.php' );

		require_once( WPAS_DOC_PATH . 'includes/integrations/awesome-support.php' );

		require_once( WPAS_DOC_PATH . 'includes/install.php' );

		if ( is_admin() ) {
			add_action( 'plugins_loaded', array( 'WPAS_Documentation_Admin', 'get_instance' ), 21, 0 );
		}

		add_action( 'plugins_loaded', array( 'WPAS_Documentation', 'get_instance' ), 21, 0 );
