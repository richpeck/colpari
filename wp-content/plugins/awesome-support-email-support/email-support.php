...
define( 'WPAS_MAIL_VERSION', '5.0.2' );
...
class AS_Email_Support_Loader {
	...
	public function load() {
		...
		require_once( 'includes/functions-unassigned-notification.php' );
		...
		add_action( 'wpas_unassigned_ticket_created', array( $this, 'unassigned_ticket_created' ) );
	...
	/**
	 * Send notification once unassigned ticket is created
	 * 
	 * @param int $ticket_id
	 * 
	 * @return boolean
	 */
	public function unassigned_ticket_created( $ticket_id ) {
		
		$notify = new ASES_Unassigned_Email_Notification( $ticket_id );
		
		if ( is_wp_error( $notify ) ) {
			return $notify;
		}
		
		return $notify->notify( 'unassigned_ticket_created' );
	}
