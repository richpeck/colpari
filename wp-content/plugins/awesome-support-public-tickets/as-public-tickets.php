<?php
/**
 * @package   Awesome Support Public Tickets Addon
 * @author    TBC / The Awesome Support Team
 * @link      http://www.getawesomesupport.com
 * @copyright 2017 Awesome Support 
 *
 * Plugin Name:       Awesome Support: Public Tickets
 * Description:       A add-on for for adding public/ private tickets functions to Awesome Support
 * Version:           1.1.1
 * Author:            The Awesome Support Team
 * License:           GPL-2.0+
 * License URI:       http://www.gnu.org/licenses/gpl-2.0.txt 
 * Text Domain:       as-public-tickets
...
if (!defined("addon_publictickets_master")){
	define("addon_publictickets_master",  plugin_dir_path( __FILE__ ));
}
 ...
 */
register_activation_hook( __FILE__, array( 'AS_Publictickets_Loader', 'activate' ) );

add_action( 'plugins_loaded', array( 'AS_Publictickets_Loader', 'get_instance' ) );
/**
...
class AS_Publictickets_Loader {
	 ...
	 * @var    string
	 */
	protected $slug = 'as-public-tickets';
	...
	public static function activate() {
		if ( ! class_exists( 'Awesome_Support' ) ) {
			...
			wp_die(
				sprintf( __( 'You need Awesome Support to activate this addon. Please <a href="%s" target="_blank">install Awesome Support</a> before continuing.', 'as-public-tickets' ), esc_url( 'http://getawesomesupport.com/?utm_source=internal&utm_medium=addon_loader&utm_campaign=Addons' ) )
			);
		} else
		{
			wpas_update_option( 'pbtk_shw_flag', true, true );
		}
	}
	...
	public function init() {
		...
		if ( ! $this->is_core_active() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support to be active. Please activate the core plugin first.', 'as-public-tickets' ), $plugin_name ) );
		}
		
		if ( ! $this->is_php_version_enough() ) {
			$this->add_error( sprintf( __( 'Unfortunately, %s can not run on PHP versions older than %s. Read more information about <a href="%s" target="_blank">how you can update</a>.', 'as-public-tickets' ), $plugin_name, $this->php_version_required, esc_url( 'http://www.wpupdatephp.com/update/' ) ) );
		}
		
		if ( ! $this->is_version_compatible() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support version %s or greater. Please update the core plugin first.', 'as-public-tickets' ), $plugin_name, $this->version_required ) );
		}
	...
	**/
	...
	{
		define( 'AS_PT_VERSION', '1.1.1' );
		define( 'WPAS_PT_URL', trailingslashit( plugin_dir_url( __FILE__ ) ) );
		define( 'WPAS_PT_PATH', trailingslashit( plugin_dir_path( __FILE__ ) ) );
		define( 'WPAS_PT_ROOT', trailingslashit( dirname( plugin_basename( __FILE__ ) ) ) );
	...
	public function add_license_notice() {
		...
		WPAS()->admin_notices->add_notice( 'error', "lincense_{$this->slug}", sprintf( __( 'Please <a href="%s">fill-in your product license</a> now. If you don\'t, your copy of Awesome Support: Public Tickets <strong>will never be updated</strong>.', 'as-public-tickets' ), $link ) );
	...
	public function license_notice_meta( $plugin_meta, $plugin_file ) {
		...
		if ( plugin_basename( __FILE__ ) === $plugin_file ) {
			$plugin_meta[] = '<strong>' . sprintf( __( 'You must fill-in your product license in order to get future plugin updates. <a href="%s">Click here to do it</a>.', 'as-public-tickets' ), $license_page ) . '</strong>';
		}
	...
	public function load() {
		if ( is_admin() ) {
			require_once( addon_publictickets_master."includes/pbtk_settings.php" );
			new AS_Public_Tickets\Admin\Settings();
		}

		include_once addon_publictickets_master."includes/custom_fields.php";
		include_once addon_publictickets_master."includes/scripts.php";
		include_once addon_publictickets_master."includes/custom_functions.php";
		include_once addon_publictickets_master."includes/shortcodes.php";
		include_once addon_publictickets_master."includes/pb_tools.php";
	}
	...
	public function load_plugin_textdomain() {
		...
		$lang_dir       = WPAS_PT_ROOT . 'languages/';
		$lang_path      = WPAS_PT_PATH . 'languages/';
