 ...
 *
 * Plugin Name:       Awesome Support: FAQ
 * Plugin URI:        https://getawesomesupport.com/addons/?utm_source=internal&utm_medium=plugin_meta&utm_campaign=Addon_FAQ
 * Description:       An FAQ plugin that allows you to quickly turn any ticket into an FAQ article
 * Version:           2.0.0
 * Author:            Awesome Suport
 * Author URI:        https://getawesomesupport.com/?utm_source=internal&utm_medium=plugin_meta&utm_campaign=Addon_FAQ
 * Text Domain:       as-faq
 * License:           GPL-2.0+
 ...
 */
register_activation_hook( __FILE__, array( 'AS_FAQ_Loader', 'activate' ) );

add_action( 'plugins_loaded', array( 'AS_FAQ_Loader', 'get_instance' ) );
/**
...
class AS_FAQ_Loader {
	...
	public function declare_constants() {
		define( 'AS_FAQ_VERSION', '2.0.0' );
		define( 'AS_FAQ_URL',     trailingslashit( plugin_dir_url( __FILE__ ) ) );
		define( 'AS_FAQ_PATH',    trailingslashit( plugin_dir_path( __FILE__ ) ) );
	}
	...
	public function includes() {
		require( AS_FAQ_PATH . 'includes/functions-post-type.php' );
		require( AS_FAQ_PATH . 'includes/functions-search.php' );
		require( AS_FAQ_PATH . 'includes/functions-stats.php' );
		require( AS_FAQ_PATH . 'includes/functions-faq.php' );
		require( AS_FAQ_PATH . 'includes/functions-misc.php' );
		require( AS_FAQ_PATH . 'includes/functions-titan-framework.php' );
		require( AS_FAQ_PATH . 'includes/settings.php' );
		require( AS_FAQ_PATH . 'includes/shortcodes/faq.php' );
		require( AS_FAQ_PATH . 'includes/shortcodes/faq-search.php' );
		require( AS_FAQ_PATH . 'includes/integrations/awesome-support.php' );

		if ( is_admin() ) {
			require( AS_FAQ_PATH . 'includes/functions-quick-reply.php' );
			require( AS_FAQ_PATH . 'includes/functions-list-table.php' );
		}
	}
	...
	public static function activate() {
		...
		if ( ! class_exists( 'Awesome_Support' ) ) {
			...
			wp_die(
				sprintf( __( 'You need Awesome Support to activate this addon. Please <a href="%s" target="_blank">install Awesome Support</a> before continuing.', 'as-faq' ), esc_url( 'http://getawesomesupport.com/?utm_source=internal&utm_medium=addon_loader&utm_campaign=Addons' ) )
			);
		}
	...
	public function init() {
		...
		if ( ! $this->is_core_active() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support to be active. Please activate the core plugin first.', 'as-faq' ), $plugin_name ) );
		}

		if ( ! $this->is_php_version_enough() ) {
			$this->add_error( sprintf( __( 'Unfortunately, %s can not run on PHP versions older than %s. Read more information about <a href="%s" target="_blank">how you can update</a>.', 'as-faq' ), $plugin_name, $this->php_version_required, esc_url( 'http://www.wpupdatephp.com/update/' ) ) );
		}

		if ( ! $this->is_version_compatible() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support version %s or greater. Please update the core plugin first.', 'as-faq' ), $plugin_name, $this->version_required ) );
		}
	...
	public function license_notice() {
		...
		<div class="updated error">
			<p><?php printf( __( 'Please <a href="%s">fill-in your product license</a> now. If you don\'t, your copy of Awesome Support: FAQ <strong>will never be updated</strong>.', 'as-faq' ), $license_page ); ?>
				<a href="<?php echo $url; ?>"><small>(<?php _e( 'I do NOT want the updates, dismiss this message', 'as-faq' ); ?>)</small></a></p>
		</div>
	...
	public function license_notice_meta( $plugin_meta, $plugin_file ) {
		...
		if ( plugin_basename( __FILE__ ) === $plugin_file ) {
			$plugin_meta[] = '<strong>' . sprintf( __( 'You must fill-in your product license in order to get future plugin updates. <a href="%s">Click here to do it</a>.', 'as-faq' ), $license_page ) . '</strong>';
		}
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'as-faq', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	}
