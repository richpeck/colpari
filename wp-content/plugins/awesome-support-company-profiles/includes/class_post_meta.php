...
abstract class WPAS_CP_Post_Meta {
	
	
	protected $key = '';
	
	public $base_template_path = WPAS_CP_PATH . 'includes/templates/';
	 ...
	 * @return string
	 */
	public function itemTemplate( $name = 'item' ) {
		
		
		return $this->base_template_path . $this->key . "/{$name}.php";
	}
	...
	public function form( $args = array() ) {
		...
		?>
			...
			<div class="white-popup mfp-hide" id="<?php echo $window_id; ?>">
	
				<div class="wpas_cp_mfp_window_wrapper" data-section="<?php echo $this->key; ?>">
					<?php wp_nonce_field( $nonce['action'], $nonce['name'] ); ?>

					<input type="hidden" class="wpas_cp_form_action" value="<?php echo $action; ?>" />
	 ...
	 * @param int $post_id
	 */
	public function set_data_user( $user_id ) {
		
		$this->data_user = null;
		
		if( $user_id ) {
			$this->data_user = $user_id;
		}
