<?php
/**
 * @package   Awesome Support Filepicker
 ...
 *
 * @wordpress-plugin
 * Plugin Name:       Awesome Support: Filestack (formerly Filepicker)
 * Plugin URI:        http://getawesomesupport.com/addons/filepicker/?utm_source=internal&utm_medium=plugin_meta&utm_campaign=Addons_Filepicker
 * Description:       Switch your tickets attachments over to Filestack instead of hosting them on your site.
 * Version:           1.0.8
 * Author:            ThemeAvenue
 * Author URI:        http://themeavenue.net/?utm_source=internal&utm_medium=plugin_meta&utm_campaign=Addons_Filepicker
 * Text Domain:       as-filepicker
 * License:           GPL-2.0+
...
define( 'AS_FP_VERSION', '1.0.8' );
define( 'AS_FP_URL',     trailingslashit( plugin_dir_url( __FILE__ ) ) );
define( 'AS_FP_PATH',    trailingslashit( plugin_dir_path( __FILE__ ) ) );
 ...
 */
register_activation_hook( __FILE__, array( 'AS_Filepicker_Loader', 'activate' ) );

add_action( 'plugins_loaded', array( 'AS_Filepicker_Loader', 'get_instance' ) );
/**
...
class AS_Filepicker_Loader {
	 ...
	 * @var    string
	 */
	protected $version_required = '4.2.0';
	...
	public static function activate() {
		...
		if ( ! class_exists( 'Awesome_Support' ) ) {
			...
			wp_die(
				sprintf( __( 'You need Awesome Support to activate this addon. Please <a href="%s" target="_blank">install Awesome Support</a> before continuing.', 'as-filepicker' ), esc_url( 'http://getawesomesupport.com/?utm_source=internal&utm_medium=addon_loader&utm_campaign=Addons' ) )
			);
		}
	...
	public function init() {
		...
		if ( ! $this->is_core_active() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support to be active. Please activate the core plugin first.', 'wpas-filepicker' ), $plugin_name ) );
		}

		if ( ! $this->is_php_version_enough() ) {
			$this->add_error( sprintf( __( 'Unfortunately, %s can not run on PHP versions older than %s. Read more information about <a href="%s" target="_blank">how you can update</a>.', 'wpas-filepicker' ), $plugin_name, $this->php_version_required, esc_url( 'http://www.wpupdatephp.com/update/' ) ) );
		}

		if ( ! $this->is_version_compatible() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support version %s or greater. Please update the core plugin first.', 'wpas-filepicker' ), $plugin_name, $this->version_required ) );
		}
	...
	public function license_notice() {
		...
		<div class="updated error">
			<p><?php printf( __( 'Please <a href="%s">fill-in your product license</a> now. If you don\'t, your copy of Awesome Support: Filepicker <strong>will never be updated</strong>.', 'as-filepicker' ), $license_page ); ?>
			<a href="<?php echo $url; ?>"><small>(<?php _e( 'I do NOT want the updates, dismiss this message', 'as-filepicker' ); ?>)</small></a></p>
		</div>
	...
	public function license_notice_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
		...
		if ( plugin_basename( __FILE__ ) === $plugin_file ) {
			$plugin_meta[] = '<strong>' . sprintf( __( 'You must fill-in your product license in order to get future plugin updates. <a href="%s">Click here to do it</a>.', 'as-filepicker' ), $license_page ) . '</strong>';
		}
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'as-filepicker', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	}
	...
	public function load() {
		...
		require_once( AS_FP_PATH . 'class-as-filepicker.php' );
		...
		if ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) {
			...
			if ( is_admin() ) {
				 ...
				 * Load the settings
				 */
				require_once( AS_FP_PATH . 'settings.php' );

				/**
				 * Add link to settings tab
				 */
				add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), array( 'AS_Filepicker', 'settings_page_link' ) );
