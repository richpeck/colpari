...
class CSFramework_Option_group extends CSFramework_Options {
	...
	public function output() {
		...
		echo '<div class="cs-group hidden cs-group-template" data-group-ini="'.$last_id.'" data-group-index="'.$last_id.'">';

		echo '<div class="cs-group-title">';
		echo '<h4>'. $acc_title .'</h4>';
		echo '<div class="cs-element cs-pull-right"><a href="#" class="button cs-warning-primary cs-remove-group">'. __( 'Remove', 'cs-framework' ) .'</a></div>';
		echo '</div>';
		...
		if( ! empty( $this->value ) ) {
			foreach ( $this->value as $key => $value ) {
				...
				$unique_id 	= preg_match("/\[(\d+)\](?![\s\S]*\[(\d+)\])/", $this->unique, $unique_result);
				$group_id 	= ($unique_result[1]) ? $unique_result[1] : $key;

				$field_title = ( ! empty( $search_id ) ) ? $acc_title : $field_title;
				echo '<div class="cs-group" data-group-index="'.$group_id.'">';
				echo '<div class="cs-group-title">';
				echo '<h4>'. $field_title .': '. $title .'</h4>';
				echo '<div class="cs-element cs-pull-right"><a href="#" class="button cs-warning-primary cs-remove-group">'. __( 'Remove', 'cs-framework' ) .'</a></div>';
				echo '</div>';
				echo '<div class="cs-group-content">';
