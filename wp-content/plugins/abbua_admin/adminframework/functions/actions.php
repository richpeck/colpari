		...
		if( ! empty( $jsons ) ) {
			...
			foreach ( $jsons as $path ) {
				...
				if( is_object( $object ) ) {
					...
					echo '<div class="cs-icon-accordion-content">';
					foreach ( $object->icons as $icon ) {
						$value = "";
						if (is_object($icon)) { 
							$class 	= $icon->class;
							$icon 	= $icon->icon;
							echo '<a class="cs-icon-tooltip" data-icon="'. $icon .'" data-title="'. $icon .'"><span class="cs-icon cs-selector"><i class="'. $class .'">'.$icon.'</i></span></a>';
						} else {
