 ...
 *
 *
 * Plugin Name:       Awesome Support: Private Credentials
 * Plugin URI:        https://www.getawesomesupport.com
 * Description:       Provides a dedicated facility for encrypting and storing sensitive data (such as passwords and other credentials) directly on an Awesome Support ticket. The stored data is automatically deleted when the ticket is closed.
 * Version:           2.0.0
...
define( 'WPAS_PC_VERSION', '2.0.0' );
define( 'WPAS_PC_URL', trailingslashit( plugin_dir_url( __FILE__ ) ) );
define( 'WPAS_PC_PATH', trailingslashit( plugin_dir_path( __FILE__ ) ) );
define( 'WPAS_PC_ROOT', trailingslashit( dirname( plugin_basename( __FILE__ ) ) ) );
 ...
 */
register_activation_hook( __FILE__, array( 'AS_Private_Credentials_Loader', 'activate' ) );
add_action( 'plugins_loaded', array( 'AS_Private_Credentials_Loader', 'get_instance' ) );
 ...
 * @return void
 */
class AS_Private_Credentials_Loader {
	 ...
	 * @var    string
	 */
	protected $slug = 'private-credentials';
	...
	public static function activate() {
		...
		if ( ! class_exists( 'Awesome_Support' ) ) {
			...
			wp_die(
				sprintf( __( 'You need Awesome Support to activate this addon. Please <a href="%s" target="_blank">install Awesome Support</a> before continuing.', 'wpas-pc' ), esc_url( 'http://getawesomesupport.com/?utm_source=internal&utm_medium=addon_loader&utm_campaign=Addons' ) )
			);
		}
	...
	public function init() {
		...
		if ( ! $this->is_core_active() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support to be active. Please activate the core plugin first.', 'wpas-pc' ), $plugin_name ) );
		}
	...
	public function add_license_notice() {
		...
		WPAS()->admin_notices->add_notice(
			'error', "lincense_{$this->slug}", sprintf( __( 'Please <a href="%s">fill-in your product license</a> now. If you don\'t, your copy of Awesome Support: Private Credentials <strong>will never be updated</strong>.', 'wpas-pc' ), $link ) );
	...
	public function license_notice_meta( $plugin_meta, $plugin_file ) {
		...
		if ( plugin_basename( __FILE__ ) === $plugin_file ) {
			$plugin_meta[] = '<strong>' . sprintf( __( 'You must fill-in your product license in order to get future plugin updates. <a href="%s">Click here to do it</a>.', 'wpas-pc' ), $license_page ) . '</strong>';
		}
	...
	public function load() {
		...
		require_once( WPAS_PC_PATH . 'class-private-credentials.php' );
		require_once( WPAS_PC_PATH . 'includes/class-Crypto.php' );

		register_activation_hook( __FILE__, array( 'WPAS_PC', 'activate' ) );

		/**
		 * Load the dependencies.
		 */

		add_action( 'plugins_loaded', array( 'WPAS_PC', 'get_instance' ), 21, 0 );
