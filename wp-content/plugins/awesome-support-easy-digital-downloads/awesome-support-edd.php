 ...
 *
 * Plugin Name:       Awesome Support: Easy Digital Downloads
 * Plugin URI:        https://getawesomesupport.com/addons/?utm_source=internal&utm_medium=plugin_meta&utm_campaign=EDD
 * Description:       A boilerplate for creating add-ons for Awesome Support.
 * Version:           1.0.7
 * Author:            The Awesome Support Team
 * Author URI:        https://getawesomesupport.com/?utm_source=internal&utm_medium=plugin_meta&utm_campaign=EDD
 * Text Domain:       as-edd
 * License:           GPL-2.0+
...
define( 'AS_EDD_VERSION', '1.0.7' );
define( 'AS_EDD_URL', trailingslashit( plugin_dir_url( __FILE__ ) ) );
define( 'AS_EDD_PATH', trailingslashit( plugin_dir_path( __FILE__ ) ) );
define( 'AS_EDD_ROOT', trailingslashit( dirname( plugin_basename( __FILE__ ) ) ) );
 ...
 */
register_activation_hook( __FILE__, array( 'AS_EDD_Loader', 'activate' ) );

add_action( 'plugins_loaded', array( 'AS_EDD_Loader', 'get_instance' ) );
/**
...
class AS_EDD_Loader {
	 ...
	 * @var    string
	 */
	protected $version_required = '3.1';
	...
	public static function activate() {
		...
		if ( ! class_exists( 'Awesome_Support' ) ) {
			...
			wp_die(
				sprintf( __( 'You need Awesome Support to activate this addon. Please <a href="%s" target="_blank">install Awesome Support</a> before continuing.', 'as-edd' ), esc_url( 'http://getawesomesupport.com/?utm_source=internal&utm_medium=addon_loader&utm_campaign=Addons' ) )
			);
		}
	...
	public function init() {
		...
		if ( ! $this->is_core_active() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support to be active. Please activate the core plugin first.', 'as-edd' ), $plugin_name ) );
		}

		if ( ! $this->is_edd_active() ) {
			$this->add_error( sprintf( __( '%s requires Easy Digital Downloads to be active. Please activate EDD first.', 'as-edd' ), $plugin_name ) );
		}

		if ( ! $this->is_php_version_enough() ) {
			$this->add_error( sprintf( __( 'Unfortunately, %s can not run on PHP versions older than %s. Read more information about <a href="%s" target="_blank">how you can update</a>.', 'as-edd' ), $plugin_name, $this->php_version_required, esc_url( 'http://www.wpupdatephp.com/update/' ) ) );
		}

		if ( ! $this->is_version_compatible() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support version %s or greater. Please update the core plugin first.', 'as-edd' ), $plugin_name, $this->version_required ) );
		}
	...
	public function includes() {
		require( AS_EDD_PATH . 'includes/settings.php' );
		require( AS_EDD_PATH . 'includes/functions-edd.php' );
		require( AS_EDD_PATH . 'includes/functions-edd-sl.php' );
		require( AS_EDD_PATH . 'includes/functions-edd-fes.php' );
		require( AS_EDD_PATH . 'includes/functions-edd-purchase-history.php' );
		require( AS_EDD_PATH . 'includes/functions-refund-policy.php' );
		require( AS_EDD_PATH . 'includes/functions-misc.php' );
		require( AS_EDD_PATH . 'includes/functions-custom-fields.php' );
	}
	...
	protected function is_core_active() {
		return in_array( 'awesome-support/awesome-support.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ? true : false;
	}

	/**
	 * Check if EDD is active.
	 *
	 * Checks if EDD is listed in the active
	 * plugins in the WordPress database.
	 *
	 * @since  0.1.0
	 * @return boolean Whether or not the core is active
	 */
	protected function is_edd_active() {
	...
	public function license_notice() {
		...
		<div class="updated error">
			<p><?php printf( __( 'Please <a href="%s">fill-in your product license</a> now. If you don\'t, your copy of the plugin <strong>will never be updated</strong>.', 'as-edd' ), $license_page ); ?>
			<a href="<?php echo $url; ?>"><small>(<?php _e( 'I do NOT want the updates, dismiss this message', 'as-edd' ); ?>)</small></a></p>
		</div>
	...
	public function license_notice_meta( $plugin_meta, $plugin_file ) {
		...
		if ( plugin_basename( __FILE__ ) === $plugin_file ) {
			$plugin_meta[] = '<strong>' . sprintf( __( 'You must fill-in your product license in order to get future plugin updates. <a href="%s">Click here to do it</a>.', 'as-edd' ), $license_page ) . '</strong>';
		}
	...
	public function load_plugin_textdomain() {
		...
		$lang_dir       = AS_EDD_ROOT . 'languages/';
		$lang_path      = AS_EDD_PATH . 'languages/';
		$locale         = apply_filters( 'plugin_locale', get_locale(), 'as-edd' );
		$mofile         = "as-edd-$locale.mo";
		$glotpress_file = WP_LANG_DIR . '/plugins/awesome-support-edd/' . $mofile;

		// Look for the GlotPress language pack first of all
		if ( file_exists( $glotpress_file ) ) {
			$language = load_textdomain( 'as-edd', $glotpress_file );
		} elseif ( file_exists( $lang_path . $mofile ) ) {
			$language = load_textdomain( 'as-edd', $lang_path . $mofile );
		} else {
			$language = load_plugin_textdomain( 'as-edd', false, $lang_dir );
		}
