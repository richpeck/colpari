 ...
 *
 * @wordpress-plugin
 * Plugin Name:       Awesome Support: Envato Validation
 * Plugin URI:        http://getawesomesupport.com/addons/envato-validation/
 * Description:       Add Envato license validation for the products you sell through the marketplace. Users will have to enter their license code in order to be able to submit a ticket.
 * Version:           0.2.7
 * Author:            The Awesome Support Team
 * Author URI:        https://getawesomesupport.com
 * Text Domain:       as-envato
 * License:           GPL-2.0+
 ...
 */
register_activation_hook( __FILE__, array( 'AS_Envato_Loader', 'activate' ) );

add_action( 'plugins_loaded', array( 'AS_Envato_Loader', 'get_instance' ) );
/**
...
class AS_Envato_Loader {
	 ...
	 * @var    string
	 */
	protected $version_required = '3.2';
	 ...
	 * @var    string
	 */
	protected $slug = 'envato_validation';
	...
	public function declare_constants() {
		define( 'WPASEV_VERSION', '0.2.7' );
		define( 'WPASEV_URL', trailingslashit( plugin_dir_url( __FILE__ ) ) );
		define( 'WPASEV_PATH', trailingslashit( plugin_dir_path( __FILE__ ) ) );
		define( 'WPASEV_ROOT', trailingslashit( dirname( plugin_basename( __FILE__ ) ) ) );
	}

	public function includes() {
		require_once( WPASEV_PATH . 'class-envato-validation.php' );
		require_once( WPASEV_PATH . 'settings-envato.php' );
	}
	...
	public static function activate() {
		...
		if ( ! class_exists( 'Awesome_Support' ) ) {
			...
			wp_die(
				sprintf( __( 'You need Awesome Support to activate the Envato addon. Please <a href="%s" target="_blank">install Awesome Support</a> before continuing.', 'as-envato' ), esc_url( 'http://getawesomesupport.com/?utm_source=internal&utm_medium=addon_loader&utm_campaign=Addons' ) )
			);
		}
	...
	public function init() {
		...
		if ( ! $this->is_core_active() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support to be active. Please activate the core plugin first.', 'as-envato' ), $plugin_name ) );
		}

		if ( ! $this->is_php_version_enough() ) {
			$this->add_error( sprintf( __( 'Unfortunately, %s can not run on PHP versions older than %s. Read more information about <a href="%s" target="_blank">how you can update</a>.', 'as-envato' ), $plugin_name, $this->php_version_required, esc_url( 'http://www.wpupdatephp.com/update/' ) ) );
		}

		if ( ! $this->is_version_compatible() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support version %s or greater. Please update the core plugin first.', 'as-envato' ), $plugin_name, $this->version_required ) );
		}
	...
	public function license_notice() {
		...
		<div class="updated error">
			<p><?php printf( __( 'Please <a href="%s">fill-in your product license</a> now. If you don\'t, your copy of the plugin <strong>will never be updated</strong>.', 'as-envato' ), $license_page ); ?>
				<a href="<?php echo $url; ?>"><small>(<?php _e( 'I do NOT want the updates, dismiss this message', 'as-envato' ); ?>)</small></a></p>
		</div>
	...
	public function license_notice_meta( $plugin_meta, $plugin_file ) {
		...
		if ( plugin_basename( __FILE__ ) === $plugin_file ) {
			$plugin_meta[] = '<strong>' . sprintf( __( 'You must fill-in your product license in order to get future plugin updates. <a href="%s">Click here to do it</a>.', 'as-envato' ), $license_page ) . '</strong>';
		}
	...
	public function load() {
		add_action( 'plugins_loaded', array( 'WPAS_Envato_Validation', 'get_instance' ), 21, 0 );
	}
	...
	public function load_plugin_textdomain() {
		...
		$lang_dir       = WPASEV_ROOT . 'languages/';
		$lang_path      = WPASEV_PATH . 'languages/';
		$locale         = apply_filters( 'plugin_locale', get_locale(), 'as-envato' );
		$mofile         = "as-envato-$locale.mo";
		$glotpress_file = WP_LANG_DIR . '/plugins/awesome-support-envato/' . $mofile;

		// Look for the GlotPress language pack first of all
		if ( file_exists( $glotpress_file ) ) {
			$language = load_textdomain( 'as-envato', $glotpress_file );
		} elseif ( file_exists( $lang_path . $mofile ) ) {
			$language = load_textdomain( 'as-envato', $lang_path . $mofile );
		} else {
			$language = load_plugin_textdomain( 'as-envato', false, $lang_dir );
		}
