...
$result_data = rns_get_delay_reply_according_to_chart_type( $second, $statuses, $search_filter, $status_get, $staff_get,  $sDate_get, $eDate_get,  $state_get , $taxonomy_get , $cus_fields_get , $ticket_author );
...
$time_in = __( "Minutes" , "reports-and-statistics" );

if( rns_delay_report_check_minutes_value_in_points_array($points,$second) === true ) { // if the minutes are more that limit 
	
	$points = rns_delay_report_convert_points_data_minutes_to_hour( $points,$second );
	$time_in = __("Hours" , "reports-and-statistics" );
	
	if( rns_delay_report_check_minutes_value_in_points_array($points,$second) === true ) { // if the hours are more that limit 
	
		$points = rns_delay_report_convert_points_data_hours_to_day( $points,$second );
		$time_in = __("Days" , "reports-and-statistics" );
	
	}
}


rns_get_delay_chart_by_points_label_and_chart_type( $points , $labels , $colors ,  $second , $chart_type , $time_in );
