(function (jQuery) {
  ...
  function getTooltip(element) {
    return jQuery('#' + element.attr(a.aDes));
  }
  ...
  function getElementPosition(element) {
    ...
    return {
      left: elementPosition.left - jQuery(window).scrollLeft(),
      top: elementPosition.top - jQuery(window).scrollTop()
    };
  }
  ...
  methods.init = function (userSettings, elementWithTooltip) {
    var settings = jQuery.extend({}, jQuery.fn.ariaTooltip.defaultSettings, userSettings),
    ...
    if (settings.responsive) {
      ...
      for (i; i < l; i = i + 1) {
        ...
        responsiveSettingsArray.push(jQuery.extend({}, settings, responsiveSettings[i]));
      }
    }
  ...
  methods.show = function (elementWithTooltip) {
    var tooltip = elementWithTooltip.data('tooltipArray')[1],
      ...
      screenSize = getElementWidthAndHeight(jQuery(window)),
  ...
  //PLUGIN
  //-----------------------------------------------
  jQuery.fn.ariaTooltip = function (userSettings) {

    //init tooltip
    if (typeof userSettings === 'object' || typeof userSettings === 'undefined') {
      this.each(function () {
        methods.init(userSettings, jQuery(this));
      });
      return;
    } else {
      //call public methods
      switch (userSettings) {
        ...
        case 'destroy':
          this.each(function () {
            methods.destroy(jQuery(this));
          });
          break;
        case 'remove':
          this.each(function () {
            methods.remove(jQuery(this));
          });
          break;
      }
    }
  ...
  jQuery.fn.ariaTooltip.defaultSettings = {
    translateX: 0, //%
    translateY: 0, //%
...
jQuery(document).ready(function () {
  'use strict';
  jQuery('.has-tooltip').ariaTooltip({
    responsive: [
      {
        breakpoint: 1,
