 ...
 *
 * Plugin Name:       Awesome Support: Gravity Forms
 * Plugin URI:        https://www.getawesomesupport.com
 * Description:       Integrates Gravity Forms with Awesome Support tickets.
 * Version:           1.5.0
...
define( 'WPAS_GF_VERSION', '1.5.0' );
define( 'WPAS_GF_URL', trailingslashit( plugin_dir_url( __FILE__ ) ) );
define( 'WPAS_GF_PATH', trailingslashit( plugin_dir_path( __FILE__ ) ) );
define( 'WPAS_GF_ROOT', trailingslashit( dirname( plugin_basename( __FILE__ ) ) ) );
 ...
 */
register_activation_hook( __FILE__, array( 'AS_GF_Loader', 'activate' ) );
add_action( 'plugins_loaded', array( 'AS_GF_Loader', 'get_instance' ), 10 );
...
class AS_GF_Loader {
	...
	public function init() {
		...
		if( ! $this->is_core_active() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support to be active. Please activate the core plugin first.', 'wpas-gf' ), $plugin_name ) );
		}

		if( ! $this->is_php_version_enough() ) {
			$this->add_error( sprintf( __( 'Unfortunately, %s can not run on PHP versions older than %s. Read more information about <a href="%s" target="_blank">how you can update</a>.', 'wpas-gf' ), $plugin_name, $this->php_version_required, esc_url( 'http://www.wpupdatephp.com/update/' ) ) );
		}

		if( ! $this->is_version_compatible() ) {
			$this->add_error( sprintf( __( '%s requires Awesome Support version %s or greater. Please update the core plugin first.', 'wpas-gf' ), $plugin_name, $this->version_required ) );
		}
	...
	public function add_license_notice() {
		...
		WPAS()->admin_notices->add_notice(
			'error', "license_{$this->slug}", sprintf( __( 'Please <a href="%s">fill-in your product license</a> now. If you don\'t, your copy Awesome Support: Gravity Forms <strong>will never be updated</strong>.', 'wpas-gf' ), $link ) );
	}
	...
	public static function activate() {
		...
		if( ! class_exists( 'Awesome_Support' ) ) {
			...
			wp_die(
				sprintf( __( 'You need Awesome Support to activate this addon. Please <a href="%s" target="_blank">install Awesome Support</a> before continuing.', 'wpas-gf' ), esc_url( 'http://getawesomesupport.com/?utm_source=internal&utm_medium=addon_loader&utm_campaign=Addons' ) )
			);
		}
	}
	...
	public function license_notice_meta( $plugin_meta, $plugin_file ) {
		...
		if( plugin_basename( __FILE__ ) === $plugin_file ) {
			$plugin_meta[] = '<strong>' . sprintf( __( 'You must fill-in your product license in order to get future plugin updates. <a href="%s">Click here to do it</a>.', 'wpas-gf' ), $license_page ) . '</strong>';
		}
	...
	public function load() {
		...
		require_once( WPAS_GF_PATH . 'class-gravity-forms.php' );
		register_activation_hook( __FILE__, array( 'WPAS_GF', 'activate' ) );

		/**
		 * Load the settings
		 */
		require_once( WPAS_GF_PATH . 'settings.php' );

		/**
		 * Load the dependencies.
		 */
		require_once( WPAS_GF_PATH . 'includes/class-ticket.php' );
		require_once( WPAS_GF_PATH . 'includes/class-mappings.php' );

		add_action( 'plugins_loaded', array( 'WPAS_GF', 'get_instance' ), 21, 0 );
